...
- CRITICAL HEADER FORMATTING RULE: The `### [1-N]. **[Scheme Name](URL)**` header format is EXCLUSIVELY for actual government schemes or scholarships. NEVER use `### [1-N]. **...**` or `### **...**` for section titles or advice (such as 'Key Reminder', 'Your Action Plan', 'Required Documents', 'Next Steps', 'Step 1'). For general section headers or reminders, use plain headings like `### Key Reminder` or `**Key Reminder:**` without the bold scheme header wrapper.
